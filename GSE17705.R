...
GSE17705 <- getGEO('GSE17705', GSEMatrix = TRUE, getGPL = FALSE)
exprSet <- exprs(GSE17705[[1]])

par(mar=c(4,4,2,1))
title <- paste ("GSE17705", "/", annotation(GSE17705[[1]]), " value distribution", sep ="")
...
Clinical_info = pData(GSE17705[[1]])
colnames(Clinical_info)

Clinical_info = Clinical_info[colnames(Markers_expr),c(41, 44)]   
...
ggsurv <- ggsurvplot(fit, pval = TRUE, conf.int = FALSE,
                     ...
                     legend.title=c(""),
                     legend.labs=c("SUCLA2 Low (n = 64)","SUCLA2 High (n = 234)"),
                     #font.legend=c(20),
...
ggsurv <- ggsurvplot(fit, pval = TRUE, conf.int = FALSE,
                     ...
                     risk.table = F, legend=c(0.4,0.2),
                     legend.title=c(""),
                     legend.labs=c("USP10 Low (n = 97)","USP10 High (n = 201)"),
                     #font.legend=c(20),
                     pval.coord=c(2,0.35), pval.size = 10)
dev.new()
ggsurv
...
ggsurv <- ggsurvplot(fit, pval = TRUE, conf.int = FALSE,
                     ...
                     risk.table = F, legend=c(0.4,0.2),
                     legend.title=c(""),
                     legend.labs=c("SUCLA2*USP10 Low (n = 184)","SUCLA2*USP10 High (n = 114)"),
                     #font.legend=c(20),
                     pval.coord=c(2,0.35), pval.size = 10)
dev.new()
ggsurv
